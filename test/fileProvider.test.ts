...
vi.mock('vscode', () => ({
  ...
  workspace: {
    ...
    getWorkspaceFolder: () => ({ name: 'workspace', uri: { fsPath: 'C:\\workspace' } }),
  },
  TreeItem: class {
    description?: string;
...
describe('FileProvider', () => {
  ...
  it('coalesces initial scans and applies later file changes incrementally', async () => {
    ...
    finishScan?.([uri('C:/workspace/docs/a.md')]);
    await Promise.all([first, second]);

    expect(findFiles).toHaveBeenCalledTimes(1);

    provider.addFiles([
      uri('C:/workspace/docs/b.md') as never,
      uri('C:/workspace/docs/ignored.ts') as never,
    ]);
    expect(findFiles).toHaveBeenCalledTimes(1);
  ...
  it('updates descendants when a folder is renamed or deleted', async () => {
    findFiles.mockResolvedValue([
      uri('C:/workspace/old/a.md'),
      uri('C:/workspace/old/nested/b.md'),
    ]);
  ...
  it('does not rebuild the complete tree when the active file is reported twice', async () => {
    ...
    const provider = new FileProvider(context as never);
    await provider.getChildren();
    const active = uri('C:/workspace/docs/a.md') as never;
